...
$tujuan             = $_POST['tujuan'];
$keluarperangkat    = $_POST['keluarperangkat'];
$masukperangkat     = $_POST['masukperangkat'];
...
$sql = "INSERT INTO sikupat (nama, instansi, kotakab, jenisidentitas, noidentitas, nobadge, tanggal, jammasuk, jamkeluar, tujuan, keluarperangkat, masukperangkat, ket, ttd, foto)
        VALUES ('$nama', '$instansi', '$kotakab', '$jenisidentitas', '$noidentitas', '$nobadge', '$tanggal', '$jammasuk', '$jamkeluar', '$tujuan', '$keluarperangkat', '$masukperangkat', '$keterangan', '$targetFileTtd', '$targetFileGambar')";

if ($koneksi->query($sql) === TRUE) {
    echo '<script>alert("Terimakasih ' . $nama . ' sudah mengisi daftar hadir. Jangan lupa untuk berkunjung kembali."); window.location.replace("index.php?timestamp=" + new Date().getTime());</script>';
} else {
